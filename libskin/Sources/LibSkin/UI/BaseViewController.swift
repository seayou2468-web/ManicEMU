...
class BaseViewController: UIViewController {
    ...
    lazy var closeButton: SymbolButton = {
        let view = SymbolButton(image: UIImage(symbol: .xmark, font: UIFont.systemFont.body(weight: .bold)), enableGlass: true)
    ...
    func configStyle() {
        ...
        if fullScreen {
            ...
            sheetPresentationController.preferredCornerRadius = CGSize.zero.CornerRadiusMax
        }
    }
    ...
    override func viewDidLoad() {
        print("\(String(describing: Self.self)) viewDidLoad")
        super.viewDidLoad()
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: any UIViewControllerTransitionCoordinator) {
        ...
        setPreferredContentSize(windowSize: CGSize(width: CGSize.zero.WindowSize.height, height: CGSize.zero.WindowSize.width))//这个常量的反应会比较慢 所以获取反向值
    }

    fileprivate func setPreferredContentSize(windowSize: CGSize = CGSize.zero.WindowSize) {
        if UIDevice.isPad {
