...
class FlexSkinSettingViewController: BaseViewController {
    ...
    private lazy var firstTimeGuideView: UIView = {
        ...
        let imageView = UIImageView(image: UIImage(named: "flex_guide", in: .libSkin, with: nil))
        view.addSubview(imageView)
        imageView.snp.makeConstraints { make in
            make.center.equalToSuperview()
        }
        let label = UILabel()
        label.font = UIFont.systemFont.body(size: .l)
        label.textColor = UIColor.label
        label.text = Bundle.libSkinLocalizedString(forKey: "flexSkinGuideTitle")
        view.addSubview(label)
        label.snp.makeConstraints { make in
            make.centerX.equalToSuperview()
            make.top.equalTo(imageView.snp.bottom).offset(CGSize.zero.ContentSpaceMin)
        }
        view.addTapGesture { [weak self] gesture in
            guard let self = self else { return }
            self.firstTimeGuideView.isHidden = true
            self.firstTimeGuideView.removeFromSuperview()
            UserDefaults.standard.set(true, forKey: LibSkinConstants.DefaultKey.FlexSkinFirstTimeGuide)
        }
    ...
    private lazy var resetButton: SymbolButton = {
        let view = SymbolButton(image: nil,
                                title: Bundle.libSkinLocalizedString(forKey: "controllerMappingReset"),
                                titleFont: UIFont.systemFont.body(size: .m),
                                edgeInsets: UIEdgeInsets(top: 0, left: CGSize.zero.ContentSpaceTiny, bottom: 0, right: CGSize.zero.ContentSpaceTiny),
                                titlePosition: .left,
                                enableGlass: true)
    ...
    private lazy var backgroundImageButton: SymbolButton = {
        let view = SymbolButton(image: nil,
                                title: Bundle.libSkinLocalizedString(forKey: "background"),
                                titleFont: UIFont.systemFont.body(size: .m),
                                edgeInsets: UIEdgeInsets(top: 0, left: CGSize.zero.ContentSpaceTiny, bottom: 0, right: CGSize.zero.ContentSpaceTiny),
                                titlePosition: .left,
                                enableGlass: true)
        ...
        view.addTapGesture { [weak self] gesture in
            ...
            let fetchImageActions = [
                UIAction(title: Bundle.libSkinLocalizedString(forKey: "readyEditCoverTakePhoto"),
                ...
                UIAction(title: Bundle.libSkinLocalizedString(forKey: "readyEditCoverAlbum"),
                ...
                UIAction(title: Bundle.libSkinLocalizedString(forKey: "readyEditCoverFile"),
            ...
            if let background = self.background {
                actions.append(UIMenu(title: isLandScape ? Bundle.libSkinLocalizedString(forKey: "addBackgroundForLandscape") : Bundle.libSkinLocalizedString(forKey: "addBackgroundForPortrait"), options: .singleSelection, children: fetchImageActions))

                if let backgroundImage {
                    actions.append(UIAction(title: Bundle.libSkinLocalizedString(forKey: "editTitle") + Bundle.libSkinLocalizedString(forKey: "background")) { [weak self] _ in
                        guard let self else { return }
                        //编辑背景
                ...
                actions.append(UIMenu(title: Bundle.libSkinLocalizedString(forKey: "updateBackground"), options: .singleSelection, children: [
                    UIAction(title: Bundle.libSkinLocalizedString(forKey: "updateBackgroundForGame"),
                    ...
                    UIAction(title: String(format: Bundle.libSkinLocalizedString(forKey: "updateBackgroundForConsole"), self.game.gameType.localizedShortName),
                    ...
                    UIAction(title: Bundle.libSkinLocalizedString(forKey: "updateBackgroundForGlobal"),
                ...
                if background.games.contains(self.game.id) {
                    removeActions.append(UIAction(title: Bundle.libSkinLocalizedString(forKey: "removeBackgroundForGame"),
                                                  state: .off,
                                                  handler: { [weak self] _ in
                        guard let self else { return }
                        //移除当前游戏背景
                ...
                if background.consoles.contains(self.game.gameType.localizedShortName) {
                    removeActions.append(UIAction(title: String(format: Bundle.libSkinLocalizedString(forKey: "removeBackgroundForConsole"), self.game.gameType.localizedShortName),
                                                  state: .off,
                                                  handler: { [weak self] _ in
                        guard let self else { return }
                        //移除平台背景
                ...
                if background.global {
                    removeActions.append(UIAction(title: Bundle.libSkinLocalizedString(forKey: "removeBackgroundForGlobal"),
                                                  state: .off,
                                                  handler: { [weak self] _ in
                        guard let self else { return }
                        //移除全局背景
                ...
                if removeActions.count > 0 {
                    actions.append(UIMenu(title: Bundle.libSkinLocalizedString(forKey: "removeTitle") + Bundle.libSkinLocalizedString(forKey: "background"), options: .singleSelection, children: removeActions))
                }
    ...
    override func viewDidLoad() {
        ...
        resetButton.snp.makeConstraints { make in
            make.leading.equalToSuperview().offset(CGSize.zero.ContentSpaceMid)
            make.top.equalToSuperview().offset(CGSize.zero.SafeAera.top == 0 ? 20 : CGSize.zero.SafeAera.top)
            make.height.equalTo(CGSize.zero.ItemHeightUltraTiny)
        ...
        backgroundImageButton.snp.makeConstraints { make in
            make.leading.equalTo(resetButton.snp.trailing).offset(CGSize.zero.ContentSpaceMin)
        ...
        if !UserDefaults.standard.bool(forKey: LibSkinConstants.DefaultKey.FlexSkinFirstTimeGuide) {
            ...
            do {
                ...
                if let originalInfoEntry = archive["info_flex.json"] {
                    ...
                    if let originalDataForCurrentTraits = try getValueFromJSON(originalInfoData, keyPath: resetInfoPath), let currentInfoEntry = archive["info.json"] {
                        ...
                        let tempUrl = URL(fileURLWithPath: LibSkinConstants.Path.Temp.appendingPathComponent(skinPath.lastPathComponent))
            ...
            do {
                ...
                for (index, view) in containerViews.enumerated() {
                    if initFrames[index].rounded() != view.frame.rounded() {
                        print("初始frame:\(initFrames[index]) view的frame:\(view.frame) 有变更，进行皮肤更新")
                        isModified = true
                    }
                }
                ...
                if let _ = archive["info_flex.json"] {} else {
                    //备份不存在 先创建一个备份
                    print("备份info.json为info_flex.json")
                ...
                print("生成info.json的访问路径:\(infoPath)")
                ...
                print("生成screens信息:\(newScreens)")

                //修改screen信息
                print("修改screens的路径是:\(infoPath + ["screens"])")
    ...
    private func modifyJSONData(_ jsonData: Data, keyPath: [String], newValue: Any) throws -> Data {
        ...
        modify(object: &jsonDecodable, keys: keyPath[...], newValue: newValue)

        // 3. 转回Data
        let updatedData = try JSONSerialization.data(withJSONObject: jsonDecodable, options: [])
        return updatedData
    }
