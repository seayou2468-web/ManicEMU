...
public struct SomeInput: Input, Codable, Hashable
{
    ...
    public init(stringValue: String, intValue: Int?, type: InputType, isContinuous: Bool? = nil, itemID: String? = nil)
    {
        ...
        else
        {
            ...
            case .game(let gameType):
                guard let deltaCore = LibSkin.core(for: gameType), let input = deltaCore.gameInputType.init(stringValue: self.stringValue) else { break }
...
public extension SomeInput
{
    ...
    init(from decoder: Decoder) throws
    {
        ...
        case .game(let gameType):
            guard let deltaCore = LibSkin.core(for: gameType), let input = deltaCore.gameInputType.init(stringValue: stringValue) else {
