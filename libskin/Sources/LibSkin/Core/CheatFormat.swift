...
public struct CheatFormat: Hashable
{
    ...
    public init(name: String, format: String, type: CheatType, allowedCodeCharacters: CharacterSet = CharacterSet.hexCharacterSet)
    {
        self.name = name
        self.format = format
        self.type = type
...
{
    func purify(with characterSet: CharacterSet) -> String
    {
    ...
    func formatted(with cheatFormat: CheatFormat) -> String
    {
        ...
        let sanitizedFormat = cheatFormat.format.purify(with: characterSet)
