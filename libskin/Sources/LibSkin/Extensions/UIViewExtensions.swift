...
extension UIView {
    @discardableResult
    func makeBlur(blurRadius: CGFloat = 12.5, blurColor: UIColor = UIColor.systemBackground, blurAlpha: CGFloat = 0.9, cornerRadius: CGFloat? = nil) -> VisualEffectView {
        removeBlur()
    ...
    func makeShadow(ofColor: UIColor = UIColor.black, offset: CGSize = .zero, radius: CGFloat = 10, opacity: Float = 0.5) {
    ...
    static func makeToast(message: String, isRemovable: Bool = true, identifier: String? = nil, duration: TimeInterval = 3, hideCompletion: (()->Void)? = nil) {
        func setupToast(toast: ToastTarget, resue: Bool = false) {
            ...
            let maxWidth = CGSize.zero.WindowWidth - 2*CGSize.zero.ContentSpaceMax
            let insets = UIEdgeInsets(horizontal: CGSize.zero.ContentSpaceHuge*2, vertical: 14*2)
            toast.config.cardEdgeInsets = insets
            let font = UIFont.systemFont.title(size: .s, weight: .semibold)
            ...
            toast.config.cardCornerRadius = cornerRadius > 24 ? CGSize.zero.CornerRadiusMax : cornerRadius
            ...
            toast.contentView.layerBorderColor = UIColor.separator
            toast.contentView.layerBorderWidth = 1
            toast.config.dynamicBackgroundColor = UIColor.systemBackground.withAlphaComponent(0.95)
        }
    ...
    static func makeLoading(timeout: Double = .infinity) {
        ...
        Alert(.identifier(LoadingIdentifier).duration(timeout)) { alert in
            ...
            alert.contentView.layerBorderColor = (UIScreen.main.traitCollection.userInterfaceStyle == .dark) ? UIColor.separator.forceStyle(.dark) : UIColor.separator.forceStyle(.light)
            alert.contentView.layerBorderWidth = 1
            alert.config.cardCornerRadius = CGSize.zero.CornerRadiusMid
            alert.config.contentViewMask { mask in }
            let blur = VisualEffectView()
            blur.blurRadius = 12.5
            blur.colorTint = (UIScreen.main.traitCollection.userInterfaceStyle == .dark) ? UIColor.systemBackground.forceStyle(.dark) : UIColor.systemBackground.forceStyle(.light)
            ...
            let pacman = UIView()
            let activity = NVActivityIndicatorView(frame: .zero, type: .pacman, color: (UIScreen.main.traitCollection.userInterfaceStyle == .dark) ? UIColor.label.forceStyle(.dark) : UIColor.label.forceStyle(.light), padding: nil)
            pacman.addSubview(activity)
            activity.snp.makeConstraints { make in
                make.centerY.equalToSuperview()
                make.centerX.equalToSuperview().offset(CGSize.zero.ContentSpaceTiny)
    ...
    static func makeAlert(identifier: String? = nil,
                          ...
                          cancelTitle: String = Bundle.libSkinLocalizedString(forKey: "cancelTitle"),
        ...
        func setupSheet(_ sheet: SheetTarget) {
            ...
            let view = UIView()
            let grabber = UIImageView(image: UIImage(named: "grabber_icon", in: .libSkin, with: nil))
            ...
            grabber.snp.makeConstraints { make in
                ...
                make.height.equalTo(CGSize.zero.ContentSpaceTiny*3)
            }
            ...
            titleLabel.font = UIFont.systemFont.title(size: .s, weight: .semibold)
            ...
            style.lineSpacing = CGSize.zero.ContentSpaceUltraTiny
            style.alignment = detailAlignment
            detailLabel.attributedText = NSAttributedString(string: detail, attributes: [.font: UIFont.systemFont.body(size: .m), .foregroundColor: UIColor.label, .paragraphStyle: style])
            containerView.addSubview(detailLabel)
            detailLabel.snp.makeConstraints { make in
                make.leading.trailing.equalToSuperview().inset(CGSize.zero.ContentSpaceHuge)
                make.top.equalTo(titleLabel.snp.bottom).offset(CGSize.zero.ContentSpaceMin)
            }
            ...
            buttonContainerView.snp.makeConstraints { make in
                make.height.equalTo(CGSize.zero.ItemHeightMid)
                make.leading.trailing.equalToSuperview()
                make.top.equalTo(line.snp.bottom)
                make.bottom.equalToSuperview().offset(-CGSize.zero.ContentInsetBottom)
            }
            ...
            if confirmTitle == nil {
                ...
                button1.font = UIFont.systemFont.title(size: .s, weight: .regular)
                button1.textColor = UIColor.secondaryLabel
                ...
                verticalLine.backgroundColor = UIColor.separator
                buttonContainerView.addSubview(verticalLine)
                verticalLine.snp.makeConstraints { make in
                    make.size.equalTo(CGSize(width: 1, height: 26))
                    make.centerX.equalToSuperview()
                    make.top.equalToSuperview().offset(CGSize.zero.ContentSpaceMin)
                }
                ...
                button2.font = UIFont.systemFont.title(size: .s, weight: .semibold)
                button2.textColor = UIColor.systemRed
...
extension UIView {
    ...
    private func addOverlay() {
        ...
        if let _ = self.overlayView {
            ...
            view.backgroundColor = UIColor.systemBackground.reverseStyle().withAlphaComponent(0.08)
            view.isHidden = true
