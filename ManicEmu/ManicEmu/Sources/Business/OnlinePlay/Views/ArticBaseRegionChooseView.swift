//
//  ArticBaseRegionChooseView.swift
//  ManicEmu
//
...
class ArticBaseRegionChooseView: BaseView {
    static func show(completion: ((_ region: String?)->Void)? = nil ) {
        Sheet { sheet in
            ...
            detailLabel.text = R.string.localizable.chooseRegion()
            ...
            let chooseView = ArticBaseRegionChooseView()
            chooseView.didSelected = { [weak sheet] region in
                sheet?.pop {
                    completion?(region)
                }
            }
            ...
            chooseView.snp.makeConstraints { make in
                ...
                let count = CGFloat(Constants.Strings.ThreeDSHomeMenuRegions.count)
    ...
    var didSelected: ((_ region: String?)->Void)? = nil
    ...
    private let regions = Constants.Strings.ThreeDSHomeMenuRegions
...
extension ArticBaseRegionChooseView: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.setData(title: regions[indexPath.row])
        return cell
    }
}

extension ArticBaseRegionChooseView: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        didSelected?(regions[indexPath.row])
    }
}
