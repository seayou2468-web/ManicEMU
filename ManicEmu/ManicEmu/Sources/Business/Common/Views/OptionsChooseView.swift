...
class OptionsChooseView: BaseView {
    static func show(options: [String],
        ...
        Sheet { sheet in
            ...
            let chooseView = OptionsChooseView(options: options)
            chooseView.didSelected = { [weak sheet] index in
                sheet?.pop {
                    completion?(index)
                }
            }
    ...
    var didSelected: ((_ region: Int)->Void)? = nil
...
extension OptionsChooseView: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.setData(title: options[indexPath.row])
        return cell
    }
}

extension OptionsChooseView: UICollectionViewDelegate {
