...
class SelectableCollectionKeyHandler: InjectableResponder {
    ...
    static let upCommand    = DiscoverableKeyCommand(([], .upArrow),    action: #selector(kbd_moveUp),    title: localisedString(.collection_moveUp),    allowsAutomaticMirroring: false)
    static let downCommand  = DiscoverableKeyCommand(([], .downArrow),  action: #selector(kbd_moveDown),  title: localisedString(.collection_moveDown),  allowsAutomaticMirroring: false)
    static let leftCommand  = DiscoverableKeyCommand(([], .leftArrow),  action: #selector(kbd_moveLeft),  title: localisedString(.collection_moveLeft),  allowsAutomaticMirroring: false)
    static let rightCommand = DiscoverableKeyCommand(([], .rightArrow), action: #selector(kbd_moveRight), title: localisedString(.collection_moveRight), allowsAutomaticMirroring: false)
    static let aKeyCommand = DiscoverableKeyCommand(([], "z"), action: #selector(kbd_press), title: "A", allowsAutomaticMirroring: false)
    static let bKeyCommand = DiscoverableKeyCommand(([], "x"), action: #selector(kbd_press), title: "B", allowsAutomaticMirroring: false)
    static let enterKeyCommand = DiscoverableKeyCommand(([], .returnOrEnter), action: #selector(kbd_press), title: "A", allowsAutomaticMirroring: false)
    static let spaceKeyCommand = DiscoverableKeyCommand(([], .space), action: #selector(kbd_press), title: "A", allowsAutomaticMirroring: false)
...
//                    commands += activateSelectionKeyCommands
            ...
            commands += [Self.aKeyCommand, Self.bKeyCommand, Self.enterKeyCommand, Self.spaceKeyCommand]
        }
    ...
    @objc private func kbd_press(_ sender: UIKeyCommand) { self.press(sender) }
    ...
    public func press(_ sender: UIKeyCommand) {
        if let collectionView = collection as? KeyboardCollectionView {
            if sender.title == "A" {
                collectionView.buttonPress?(.a)
            } else if sender.title == "B" {
                collectionView.buttonPress?(.b)
            }
        }
    }
}
...
fileprivate var lastScrollIndexPath: IndexPath? = nil
private extension SelectableCollection {
    ...
    func selectAndShowItemAtIndexPath(_ indexPath: IndexPath, extendSelection isExtendingSelection: Bool) {
        ...
        if let lastScrollIndexPath, lastScrollIndexPath.section != indexPath.section {
            scrollToItem(at: indexPath, at: [.top, .left], animated: UIAccessibility.isReduceMotionEnabled == false)
        } else if let collectionView = self as? KeyboardCollectionView,
            let isOrthogonal = collectionView.isOrthogonalSection?(indexPath.section),
            isOrthogonal {
            //嵌套collectionView的处理
            if let cell = collectionView.cellForItem(at: indexPath) {
                scrollToCenter(subview: cell)
            } else {
                scrollToItem(at: indexPath, at: [.top, .left], animated: UIAccessibility.isReduceMotionEnabled == false)
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.35, execute: {
                    if let cell = collectionView.cellForItem(at: indexPath) {
                        self.scrollToCenter(subview: cell)
                    }
                })
            }
        } else {
        ...
        lastScrollIndexPath = indexPath
    }
    
    func scrollToCenter(subview: UICollectionViewCell, animated: Bool = true) {
        if let scrollView = subview.superview as? UIScrollView {
            scrollView.layoutIfNeeded()
            
            // 获取子视图在scrollView中的frame
            let targetFrame = scrollView.convert(subview.frame, from: subview.superview)
            
            // 获取scrollView的宽度，和inset（用于计算可见区域中心）
            let scrollViewWidth = scrollView.bounds.width
            let insetLeft = scrollView.adjustedContentInset.left
            let insetRight = scrollView.adjustedContentInset.right

            // 可见区域的中心点
            let visibleCenterX = (scrollViewWidth - insetLeft - insetRight) / 2

            // 目标contentOffset.x
            var offsetX = targetFrame.midX - visibleCenterX - insetLeft
