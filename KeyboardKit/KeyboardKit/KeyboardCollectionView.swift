...
open class KeyboardCollectionView: UICollectionView, ResponderChainInjection {
    ...
    public var isOrthogonalSection: ((_ section: Int)->Bool)? = nil
    
    public var buttonPress: ((_ key: UIControllerKey)->Void)? = nil
...
extension KeyboardCollectionView: UIControllerPressable {
    public func didControllerPress(key: UIControllerKey) {
        switch key {
        case .left:
            selectableCollectionKeyHandler.updateSelectionFromKeyCommand(SelectableCollectionKeyHandler.leftCommand)
        case .right:
            selectableCollectionKeyHandler.updateSelectionFromKeyCommand(SelectableCollectionKeyHandler.rightCommand)
        case .up:
            selectableCollectionKeyHandler.updateSelectionFromKeyCommand(SelectableCollectionKeyHandler.upCommand)
        case .down:
            selectableCollectionKeyHandler.updateSelectionFromKeyCommand(SelectableCollectionKeyHandler.downCommand)
        case .a:
            self.buttonPress?(.a)
        case .b:
            self.buttonPress?(.b)
...
private extension UICollectionViewCompositionalLayout {
    private func nextIndexPathInOrthogonalSection(_ indexPath: IndexPath, direction: NavigationDirection) -> IndexPath? {
        // 获取该 section 的所有 items
        let items = collectionView!.numberOfItems(inSection: indexPath.section)
        var item = indexPath.item
        switch direction {
        case .left:
            item = item == 0 ? items - 1 : max(0, item - 1)
        case .right:
            item = item == items - 1 ? 0 : min(items - 1, item + 1)
        default:
            break
        }
    ...
    private func nextIndexPathInMainAxis(from indexPath: IndexPath, direction: NavigationDirection) -> IndexPath? {
        let sectionCount = collectionView!.numberOfSections
        var section = indexPath.section
        let item = indexPath.item
        switch direction {
        case .up:
            section = section == 0 ? sectionCount - 1 : max(0, section - 1)
            let items = collectionView!.numberOfItems(inSection: section)
            return IndexPath(item: min(item, items - 1), section: section)
        case .down:
            section = section == sectionCount-1 ? 0 : min(sectionCount - 1, section + 1)
            let items = collectionView!.numberOfItems(inSection: section)
            return IndexPath(item: min(item, items - 1), section: section)
        default:
            return nil
    ...
    override func kbd_indexPathFromIndexPath(_ indexPath: IndexPath?, inDirection rawDirection: Int, step rawStep: Int) -> IndexPath? {
        ...
        if let indexPath {
            ...
            if let collectionView = collectionView as? KeyboardCollectionView,
                let isOrthogonal = collectionView.isOrthogonalSection?(indexPath.section),
                isOrthogonal {
                // 在 orthogonal section 内部
                if direction == .left || direction == .right {
                    // 横向移动
                    return nextIndexPathInOrthogonalSection(indexPath, direction: direction)
                } else if direction == .up || direction == .down {
                    //这里默认一个section只有一行，这个其实不应该 但是Manic的设计是强制一个section只有一行 所以这里没问题，如果未来一个section中有多行的话，这里需要进行修改
                    return nextIndexPathInMainAxis(from: indexPath, direction: direction)
                }
            } else {
                return super.kbd_indexPathFromIndexPath(indexPath, inDirection: rawDirection, step: rawStep)
            }
